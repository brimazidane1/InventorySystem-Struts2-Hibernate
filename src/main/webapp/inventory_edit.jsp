...
<body>
	...
	<h1 align="center">Edit Inventory</h1>
	<s:if test="sm != null">
	...
	<s:form action="updateInventory" method="post">
		<s:hidden value="%{inventory.id}" name="inventory.id" />
		<s:hidden value="%{inventory.itemId}" name="inventory.itemId" />
		<s:textfield name="inventory.qty" label="Inventory Qty" />
		<s:select label="Inventory Type"
		    name="inventory.type"
		    list="{'T','W'}" 
		    value="%{inventory.type}" disabled="true" />
